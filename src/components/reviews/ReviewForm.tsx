  ...
  const uploadImages = async (): Promise<string[]> => {
    ...
    try {
      for (const file of images) {
        ...
        const fileName = user 
          ? `${user.id}/${Date.now()}-${Math.random().toString(36).substring(7)}.${fileExt}`
          : `guest/${Date.now()}-${Math.random().toString(36).substring(7)}.${fileExt}`;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        reviewData.reviewer_type = 'user';
        reviewData.status = 'approved'; // 用户评价默认直接通过
      } else {
        reviewData.guest_name = guestName.trim();
        reviewData.guest_email = guestEmail.trim();
        reviewData.reviewer_type = 'guest';
        reviewData.status = 'pending'; // 访客评价需要审核
      }
      ...
      toast.success(user ? "评价提交成功" : "评价已提交，正在审核中");
      onSuccess?.();
      onClose();
      resetForm();
    } catch (error: any) {
        ...
        <div className="space-y-6 py-4">
          ...
          {!user && (
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="guest-mode" className="text-sm cursor-pointer">
                以访客身份评价
              </Label>
            </div>
          )}
          ...
          {isGuest && (
            <div className="space-y-4 p-4 bg-secondary rounded-lg">
              <div>
                <Label htmlFor="guest-name" className="mb-2 block">
              ...
              <div>
                <Label htmlFor="guest-email" className="mb-2 block">
