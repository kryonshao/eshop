  ...
  const loadOrders = async () => {
    try {
      ...
      if (user) {
        ...
        // 访客用户：按邮箱或 guest_id 查询
        storedGuestId = localStorage.getItem("guest_id");
        setGuestIdState(storedGuestId);
        if (guestEmail || storedGuestId) {
          const effectiveGuestId = storedGuestId || guestIdState || null;
          const guestClient = createGuestClient(effectiveGuestId, guestEmail || null);
          let guestQuery: any = guestClient.from("orders").select("*");
          if (guestEmail) {
            guestQuery = guestQuery.eq("guest_email", guestEmail);
          } else if (storedGuestId) {
            guestQuery = guestQuery.eq("guest_id", storedGuestId);
          }
          const { data, error } = await guestQuery.order("created_at", { ascending: false });
          if (error) throw error;
          ordersData = data;
        } else {
          ordersData = [];
      ...
      if (ordersData) {
        ...
        const effectiveGuestId = user ? null : storedGuestId || guestIdState || null;
        const detailClient = user
          ? supabase
          : createGuestClient(effectiveGuestId, guestEmail || null);
